...
/**
 * MCP Tool adapter for AppReviewTool that enables integration with other MCP packages.
 * Wraps the existing AppReviewTool functionality while implementing the McpTool interface.
 */
export class AppReviewMcpTool extends McpTool<InputArgsShape, OutputArgsShape> {
  private readonly telemetryService: TelemetryService;
  private readonly wrappedTool: AppReviewTool;

  /**
   * Constructs a new AppReviewMcpTool with the provided services.
  ...
  public constructor(telemetryService: TelemetryService) {
    ...
    this.wrappedTool = new AppReviewTool();
  }
  ...
  /**
   * Executes the app review tool to get API documentation and guidance.
   * @param _input Empty input object (no input required)
   * @returns Promise resolving to the app review documentation
   */
  public async exec(_input: InputArgs): Promise<CallToolResult> {
    try {
      // Send telemetry event for tool usage
      this.telemetryService.sendEvent('mobileWebAppReview', {
      ...
      return {
        ...
        content: [
          {
            type: 'text',
            text: `Failed to get app review documentation: ${error instanceof Error ? error.message : 'Unknown error'}`,
          },
        ],
      };
    }
