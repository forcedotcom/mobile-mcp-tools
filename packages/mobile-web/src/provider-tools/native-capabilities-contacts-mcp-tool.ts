...
/**
 * MCP Tool adapter for ContactsTool that enables integration with other MCP packages.
 * Wraps the existing ContactsTool functionality while implementing the McpTool interface.
 */
export class ContactsMcpTool extends McpTool<InputArgsShape, OutputArgsShape> {
  private readonly telemetryService: TelemetryService;
  private readonly wrappedTool: ContactsTool;

  /**
   * Constructs a new ContactsMcpTool with the provided services.
  ...
  public constructor(telemetryService: TelemetryService) {
    ...
    this.wrappedTool = new ContactsTool();
  }
  ...
  /**
   * Executes the contacts tool to get API documentation and guidance.
   * @param _input Empty input object (no input required)
   * @returns Promise resolving to the contacts documentation
   */
  public async exec(_input: InputArgs): Promise<CallToolResult> {
    try {
      // Send telemetry event for tool usage
      this.telemetryService.sendEvent('mobileWebContacts', {
      ...
      return {
        ...
        content: [
          {
            type: 'text',
            text: `Failed to get contacts documentation: ${error instanceof Error ? error.message : 'Unknown error'}`,
          },
        ],
      };
    }
