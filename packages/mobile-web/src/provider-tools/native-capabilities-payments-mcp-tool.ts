...
/**
 * MCP Tool adapter for PaymentsTool that enables integration with other MCP packages.
 * Wraps the existing PaymentsTool functionality while implementing the McpTool interface.
 */
export class PaymentsMcpTool extends McpTool<InputArgsShape, OutputArgsShape> {
  private readonly telemetryService: TelemetryService;
  private readonly wrappedTool: PaymentsTool;

  /**
   * Constructs a new PaymentsMcpTool with the provided services.
  ...
  public constructor(telemetryService: TelemetryService) {
    ...
    this.wrappedTool = new PaymentsTool();
  }
  ...
  /**
   * Executes the payments tool to get API documentation and guidance.
   * @param _input Empty input object (no input required)
   * @returns Promise resolving to the payments documentation
   */
  public async exec(_input: InputArgs): Promise<CallToolResult> {
    try {
      // Send telemetry event for tool usage
      this.telemetryService.sendEvent('mobileWebPayments', {
      ...
      return {
        ...
        content: [
          {
            type: 'text',
            text: `Failed to get payments documentation: ${error instanceof Error ? error.message : 'Unknown error'}`,
          },
        ],
      };
    }
