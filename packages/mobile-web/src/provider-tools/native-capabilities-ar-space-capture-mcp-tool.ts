...
/**
 * MCP Tool adapter for ArSpaceCaptureTool that enables integration with other MCP packages.
 * Wraps the existing ArSpaceCaptureTool functionality while implementing the McpTool interface.
 */
export class ARSpaceCaptureMcpTool extends McpTool<InputArgsShape, OutputArgsShape> {
  private readonly telemetryService: TelemetryService;
  private readonly wrappedTool: ArSpaceCaptureTool;

  /**
   * Constructs a new ARSpaceCaptureMcpTool with the provided services.
  ...
  public constructor(telemetryService: TelemetryService) {
    ...
    this.wrappedTool = new ArSpaceCaptureTool();
  }
  ...
  /**
   * Executes the AR space capture tool to get API documentation and guidance.
   * @param _input Empty input object (no input required)
   * @returns Promise resolving to the AR space capture documentation
   */
  public async exec(_input: InputArgs): Promise<CallToolResult> {
    try {
      // Send telemetry event for tool usage
      this.telemetryService.sendEvent('mobileWebARSpaceCapture', {
      ...
      return {
        ...
        content: [
          {
            type: 'text',
            text: `Failed to get AR space capture documentation: ${error instanceof Error ? error.message : 'Unknown error'}`,
          },
        ],
      };
    }
