...
import { ContactsMcpTool } from '../../src/provider-tools/native-capabilities-contacts-mcp-tool.js';
...
describe('ContactsMcpTool', () => {
  let mockTelemetryService: TelemetryService;
  let tool: ContactsMcpTool;
  ...
  beforeEach(() => {
    ...
    tool = new ContactsMcpTool(mockTelemetryService);
  });
  ...
  describe('Tool Properties', () => {
    ...
    it('should have correct name', () => {
      expect(tool.getName()).toBe('sfmobile-web-contacts');
    });
  });
  ...
  describe('Tool Configuration', () => {
    it('should have correct configuration', () => {
      ...
      expect(config.title).toBe('Salesforce Mobile Contacts Service LWC Native Capability');
      expect(config.description).toContain('Contacts Service');
  ...
  describe('Tool Execution', () => {
    it('should execute successfully and return content', async () => {
      ...
      expect(result.content[0].text).toContain('Contacts Service Grounding Context');
    ...
    it('should handle errors gracefully', async () => {
      ...
      expect(result.content).toBeDefined();
      expect(result.content[0].text).toContain('Failed to get contacts documentation');
