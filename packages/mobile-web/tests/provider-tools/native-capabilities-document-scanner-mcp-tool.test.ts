...
import { DocumentScannerMcpTool } from '../../src/provider-tools/native-capabilities-document-scanner-mcp-tool.js';
...
describe('DocumentScannerMcpTool', () => {
  let mockTelemetryService: TelemetryService;
  let tool: DocumentScannerMcpTool;
  ...
  beforeEach(() => {
    ...
    tool = new DocumentScannerMcpTool(mockTelemetryService);
  });
  ...
  describe('Tool Properties', () => {
    ...
    it('should have correct name', () => {
      expect(tool.getName()).toBe('sfmobile-web-document-scanner');
    });
  });
  ...
  describe('Tool Configuration', () => {
    it('should have correct configuration', () => {
      ...
      expect(config.title).toBe('Salesforce Mobile Document Scanner LWC Native Capability');
      expect(config.description).toContain('Document Scanner');
  ...
  describe('Tool Execution', () => {
    it('should execute successfully and return content', async () => {
      ...
      expect(result.content[0].text).toContain('Document Scanner Service Grounding Context');
    ...
    it('should handle errors gracefully', async () => {
      ...
      expect(result.content).toBeDefined();
      expect(result.content[0].text).toContain('Failed to get document scanner documentation');
