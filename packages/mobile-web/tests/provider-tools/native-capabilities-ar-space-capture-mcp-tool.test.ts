...
import { ARSpaceCaptureMcpTool } from '../../src/provider-tools/native-capabilities-ar-space-capture-mcp-tool.js';
...
describe('ARSpaceCaptureMcpTool', () => {
  let mockTelemetryService: TelemetryService;
  let tool: ARSpaceCaptureMcpTool;
  ...
  beforeEach(() => {
    ...
    tool = new ARSpaceCaptureMcpTool(mockTelemetryService);
  });
  ...
  describe('Tool Properties', () => {
    ...
    it('should have correct name', () => {
      expect(tool.getName()).toBe('sfmobile-web-ar-space-capture');
    });
  });
  ...
  describe('Tool Configuration', () => {
    it('should have correct configuration', () => {
      ...
      expect(config.title).toBe('Salesforce Mobile AR Space Capture LWC Native Capability');
      expect(config.description).toContain('AR Space Capture');
  ...
  describe('Tool Execution', () => {
    it('should execute successfully and return content', async () => {
      ...
      expect(result.content[0].text).toContain('AR Space Capture Service Grounding Context');
    ...
    it('should handle errors gracefully', async () => {
      ...
      expect(result.content).toBeDefined();
      expect(result.content[0].text).toContain('Failed to get AR space capture documentation');
