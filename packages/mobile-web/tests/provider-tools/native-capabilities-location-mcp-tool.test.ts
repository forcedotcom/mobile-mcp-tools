...
import { LocationMcpTool } from '../../src/provider-tools/native-capabilities-location-mcp-tool.js';
...
describe('LocationMcpTool', () => {
  let mockTelemetryService: TelemetryService;
  let tool: LocationMcpTool;
  ...
  beforeEach(() => {
    ...
    tool = new LocationMcpTool(mockTelemetryService);
  });
  ...
  describe('Tool Properties', () => {
    ...
    it('should have correct name', () => {
      expect(tool.getName()).toBe('sfmobile-web-location');
    });
  });
  ...
  describe('Tool Configuration', () => {
    it('should have correct configuration', () => {
      ...
      expect(config.title).toBe('Salesforce Mobile Location Services LWC Native Capability');
      expect(config.description).toContain('Location Service');
  ...
  describe('Tool Execution', () => {
    it('should execute successfully and return content', async () => {
      ...
      expect(result.content[0].text).toContain('Location Service Grounding Context');
    ...
    it('should handle errors gracefully', async () => {
      ...
      expect(result.content).toBeDefined();
      expect(result.content[0].text).toContain('Failed to get location documentation');
