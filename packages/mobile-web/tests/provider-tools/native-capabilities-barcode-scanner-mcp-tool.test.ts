...
import { BarcodeScannerMcpTool } from '../../src/provider-tools/native-capabilities-barcode-scanner-mcp-tool.js';
...
describe('BarcodeScannerMcpTool', () => {
  let mockTelemetryService: TelemetryService;
  let tool: BarcodeScannerMcpTool;
  ...
  beforeEach(() => {
    ...
    tool = new BarcodeScannerMcpTool(mockTelemetryService);
  });
  ...
  describe('Tool Properties', () => {
    ...
    it('should have correct name', () => {
      expect(tool.getName()).toBe('sfmobile-web-barcode-scanner');
    });
  });
  ...
  describe('Tool Configuration', () => {
    it('should have correct configuration', () => {
      ...
      expect(config.title).toBe('Salesforce Mobile Barcode Scanner LWC Native Capability');
      expect(config.description).toContain('Barcode Scanner');
  ...
  describe('Tool Execution', () => {
    it('should execute successfully and return content', async () => {
      ...
      expect(result.content[0].text).toContain('Barcode Scanner Service Grounding Context');
    ...
    it('should send telemetry event', async () => {
      ...
      expect(mockTelemetryService.sendEvent).toHaveBeenCalledWith('mobileWebBarcodeScanner', {
    ...
    it('should handle errors gracefully', async () => {
      ...
      expect(result.content).toBeDefined();
      expect(result.content[0].text).toContain('Failed to get barcode scanner documentation');
